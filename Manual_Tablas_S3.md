# 📊 Manual de Tablas Blood Bowl (Season 3 — 2025)

Este documento centraliza todas las tablas de consulta rápida para el asistente y los entrenadores.

---

## 📈 1. PROGRESIÓN Y VALORACIÓN

### Tabla de Mejora de Jugadores
| Mejora | Azar | Elegida |
| :--- | :---: | :---: |
| **Habilidad Primaria** | 3 PE | 6 PE |
| **Habilidad Secundaria** | - | 12 PE |
| **Mejora de Atributo** | 14 PE (1D8) | - |

*Nota: Al tirar **1D8** para mejora de atributo, un resultado de **8** permite al entrenador elegir libremente aumentar cualquier atributo permitido.*

### Aumento de Valoración (VAE / CTV)
| Atributo/Mejora | Coste |
| :--- | :--- |
| **Habilidad Primaria** | +20,000 |
| **Habilidad Secundaria** | +40,000 |
| **+1 AV (Armadura)** | +10,000 |
| **+1 MV (Movimiento)** | +20,000 |
| **+1 PA (Pase)** | +20,000 |
| **+1 AG (Agilidad)** | +30,000 |
| **+1 FU (Fuerza)** | +60,000 |

> [!IMPORTANT]
> **Habilidades de Élite (Temporada 3):** Las habilidades **Placar (Block)**, **Esquivar (Dodge)**, **Defensa (Guard)** y **Golpe mortífero (Mighty Blow)** tienen un recargo adicional de **+10,000 MO** sobre su coste normal de mejora.

---

## 🎲 2. TABLAS DE JUEGO

### Clima (2D6)
| 2D6 | Resultado | Efecto |
| :---: | :--- | :--- |
| **2** | **Calor Asfixiante** | 1D3 jugadores se desmayan tras la entrada (van a Reservas). |
| **3** | **Muy Soleado** | -1 a los chequeos de Pase. |
| **4-10** | **Clima Perfecto** | Sin efectos. |
| **11** | **Lluvioso** | -1 a Recoger, Atrapar e Interceptar. |
| **12** | **Ventisca** | -1 a Forzar la Marcha (GFI). Solo pases rápidos o cortos. |

### Eventos de Patada Inicial (2D6)
| 2D6 | Evento | Efecto Resumido |
| :---: | :--- | :--- |
| **2** | **Árbitro Intimidado** | Cada equipo recibe 1 Soborno gratuito. |
| **3** | **Tiempo Muerto** | Se suma/resta 1 turno al marcador de ambos. |
| **4** | **Defensa Sólida** | El pateador redespliega 1D3+3 jugadores. |
| **5** | **Patada Alta** | El receptor coloca un jugador bajo el balón. |
| **6** | **Hinchas Animan** | Tirada enfrentada (1D6+Animadoras). El ganador recibe un **apoyo ofensivo adicional** en el primer placaje de su turno. |
| **7** | **Entrenador Brillante** | Ganador de tirada (1D6+Ayudantes) gana 1 RR extra. |
| **8** | **Clima Cambiante** | Nueva tirada de clima. Si es perfecto, el balón se escora (3). |
| **9** | **Anticipación** | El receptor mueve 1D3+3 jugadores (1 casilla). |
| **10** | **¡A la Carga!** | El pateador mueve 1D3+3 jugadores (1 puede hacer Blitz). |
| **11** | **Indigestión** | Tirada enfrentada: el perdedor tiene un jugador con -1 MV/AR. |
| **12** | **Invasión de Campo** | Tirada enfrentada: 1D3 jugadores del perdedor Aturdidos. |

---

## 🚑 3. LESIONES Y DAÑOS

### Tabla de Lesiones (1D16)
| 1D16 | Resultado | Efecto |
| :---: | :--- | :--- |
| **1-8** | **Magullado** | Se pierde el resto del partido. |
| **9-10** | **Apaleado** | Se pierde este partido y el siguiente (MNG). |
| **11-12** | **Herida Grave** | PPP y lesión persistente. |
| **13-14** | **Herida Permanente** | PPP y Reducción de Atributo. |
| **15-16** | **MUERTO** | Eliminar de la plantilla. |

### Reducción de Atributos (1D6)
| 1D6 | Atributo afectado | Efecto |
| :---: | :--- | :---: |
| **1-2** | **Armadura** | -1 AR |
| **3** | **Movimiento** | -1 MV |
| **4** | **Pase** | -1 PA |
| **5** | **Agilidad** | -1 AG |
| **6** | **Fuerza** | -1 FU |

---

## 🔮 4. PLEGARIAS A NUFFLE (Tabla de Nuffle)
| Resultado | Evento | Efecto |
| :---: | :--- | :--- |
| **1** | **Trampilla Traicionera** | 1 en 1D6 al pisar: jugador retirado. |
| **2** | **Amigo del Árbitro** | Las tiradas de Protestar de 5 cuentan como 6. |
| **3** | **Puñal** | Un jugador gana el rasgo *Puñalar* (Stab). |
| **4** | **Músculos de Hierro** | Un jugador gana +1 Armadura. |
| **5** | **Nudilleras** | Un jugador gana *Golpe Mortífero* (Mighty Blow). |
| **8** | **Bendición de Nuffle** | Un jugador gana *Profesional*. |
| **10** | **Pase Perfecto** | Los pases completos dan 2 PE en lugar de 1. |
| **14** | **Pedrada** | 4+ en 1D6 derriba a un rival al azar. |
| **16** | **Entrenam. Intensivo** | Un jugador gana una habilidad Primaria elegida. |

---

## 🛠️ 5. CATEGORÍAS DE HABILIDADES (1D6+1D6)
*Lanzar 2D6 para determinar la habilidad al azar:*

| Resultado | Agilidad | General | Mutación | Pase | Fuerza | Triquiñuelas |
| :---: | :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | Atrapar | Agallas | Apariencia Asq. | Atento al Balón | Abrirse Paso | Agresor Discr. |
| **2** | Echarse a un Lado | Equilibrio Firme | Boca Monstruosa | Cañonero | Apartar | Crujir |
| **3** | En Pie de un Salto | Forcejear | Brazos Adic. | Líder | Brazo Fuerte | Dejada |
| **4** | Esprintar | Furia | Cola Prensil | Nervios de Acero | Cabeza Dura | Falta Rápida |
| **5** | Esquivar | Manos Seguras | Cuernos | Partenubes | Defensa | Furtivo |
| **6** | Pies Firmes | Placar | Dos Cabezas | Pasar | Golpe Mortífero | Jugar Sucio |
| **7** | Saltar | Zafarse | Tentáculos | Precisión | Mantenerse Firme | Perseguir |
| **8** | Zafarse | Placaje | Garras | Pase Seguro | Multiplacaje | Innovador viol. |
| **9** | - | - | - | - | - | Meter la bota |
| **10** | - | - | - | - | - | Piquete de ojos |
| **11** | - | - | - | - | - | Saboteador |
| **12** | - | - | - | - | - | Vuelo letal |
