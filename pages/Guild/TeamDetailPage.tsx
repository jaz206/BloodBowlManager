...
export const TeamDashboard: React.FC<TeamDashboardProps & { matchReports?: MatchReport[] }> = ({ team, onUpdate, onDeleteRequest, onBack, isGuest, matchReports = [] }) => {
    ...
    const specialRulesList = useMemo(() => (baseRoster?.specialRules_es || baseRoster?.specialRules || '').split(',').map(s => s.trim()).filter(Boolean), [baseRoster]);
                ...
                <div className="max-w-[1400px] mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-6">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {!isGuest && (
                                <button onClick={() => setShowQr(true)} className="bg-primary/10 border border-primary/20 text-primary px-4 py-2 rounded-xl text-[10px] font-black tracking-widest hover:bg-primary/20 transition-all uppercase flex items-center gap-2">
