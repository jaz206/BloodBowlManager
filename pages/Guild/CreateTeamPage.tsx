...
const TeamCreator: React.FC<TeamCreatorProps> = ({ onTeamCreate, initialRosterName }) => {
    ...
    const { teams, loading } = useMasterData();
    const [previewIndex, setPreviewIndex] = useState(0);

    useEffect(() => {
        if (!loading && initialRosterName && teams.length > 0) {
            const index = teams.findIndex(t => t.name === initialRosterName);
            if (index !== -1) setPreviewIndex(index);
        }
    }, [initialRosterName, teams, loading]);

    // Derived values
    const teamToPreview = teams[previewIndex] || (teams.length > 0 ? teams[0] : null);
    const rosterName = teamToPreview?.name || '';
    ...
    const handlePrevTeam = () => {
        setPreviewIndex(prev => (prev === 0 ? teams.length - 1 : prev - 1));
    };

    const handleNextTeam = () => {
        setPreviewIndex(prev => (prev === teams.length - 1 ? 0 : prev + 1));
    };

    const handleRosterSelectChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        const index = teams.findIndex(t => t.name === e.target.value);
        if (index !== -1) {
            setPreviewIndex(index);
        }
    };

    if (loading || !teamToPreview) {
        return (
            <div className="flex flex-col items-center justify-center p-20">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-amber-500 mb-4"></div>
                <p className="text-slate-400">Cargando facciones...</p>
            </div>
        );
    }
            ...
            <div className="text-center max-w-4xl mx-auto p-4 sm:p-8">
                ...
                <div className="bg-slate-900/50 p-4 sm:p-6 rounded-lg border border-slate-700 mb-8">
                    ...
                    <div className="flex justify-between items-center mb-4">
                        <button type="button" onClick={handlePrevTeam} className="bg-slate-700 hover:bg-slate-600 text-white font-bold py-2 px-4 rounded-md transition-colors">&larr; Anterior</button>
                        <h4 className="text-lg font-bold text-white text-center truncate px-2">{teamToPreview.name}</h4>
                        <button type="button" onClick={handleNextTeam} className="bg-slate-700 hover:bg-slate-600 text-white font-bold py-2 px-4 rounded-md transition-colors">Siguiente &rarr;</button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-1 flex flex-col items-center">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full items-center mb-4">
                                ...
                                <button type="button" onClick={() => setIsRadarModalOpen(true)} className="w-full rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-amber-400" aria-label="Ampliar gráfico de estadísticas">
