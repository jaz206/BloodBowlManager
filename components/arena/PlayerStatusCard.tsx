...
const SppActionIcons: React.FC<{ actions: Partial<Record<SppActionType, number>> }> = ({ actions }) => {
    ...
    const iconMap: Partial<Record<SppActionType, { icon: React.ReactElement<{ className?: string }>; title: string }>> = {
        ...
        MVP: { icon: <TdIcon />, title: 'MVP' }, // Reuse TdIcon or similar for MVP for now
    };
    ...
    return (
        <div className="flex items-center gap-1.5">
            {(Object.keys(actions) as SppActionType[]).map(action => {
                ...
                if (!count || !iconMap[action]) return null;
