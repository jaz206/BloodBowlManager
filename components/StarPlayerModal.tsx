                  ...
                  <div className="space-y-6">
                      ...
                      <div key={p.name}>
                        ...
                        <div className="mb-4">
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-1 bg-slate-900/50 p-2 rounded-md text-slate-200 text-sm">
                            <span className="font-mono">MV: {p.stats.MV}</span>
                            <span className="font-mono">FU: {p.stats.FU}</span>
                            <span className="font-mono">AG: {p.stats.AG}</span>
                            <span className="font-mono">PS: {p.stats.PS}</span>
                            <span className="font-mono">AR: {p.stats.AR}</span>
                          </div>
                        </div>
                        <div>
                          ...
                          <p className="text-sm text-slate-300">
                            {p.skills.split(', ').map((skill, index, arr) => (
                              <React.Fragment key={`${p.name}-${skill}`}>
                    ...
                    {player.skills && (
                      <div className="border-t border-slate-700 pt-4">
                        <h3 className="font-semibold text-slate-300 mb-2">Habilidades de Dúo</h3>
                        <p className="text-sm text-slate-300">
                          {player.skills.split(', ').map((skill, index, arr) => (
                            <React.Fragment key={`pair-${skill}`}>
                              <button onClick={() => handleSkillClick(skill)} className="text-sky-400 hover:text-sky-300 hover:underline focus:outline-none focus:ring-1 focus:ring-sky-300 rounded">{skill.trim()}</button>
                              {index < arr.length - 1 && ', '}
                            </React.Fragment>
                          ))}
