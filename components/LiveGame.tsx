...
export const LiveGame = ({ managedTeams, onTeamUpdate }: LiveGameProps): React.ReactElement => {
    ...
    return (
        <div className="animate-fade-in-slow">
            ...
            {isWeatherModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={() => setIsWeatherModalOpen(false)}>
                    <div className="bg-slate-800 p-6 rounded-lg shadow-xl border border-slate-700 max-w-sm w-full text-center" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-bold text-amber-400 mb-4">Generar Clima</h3>
                        <p className="text-slate-300 mb-6">Tira 2D6 para determinar el clima.</p>
                        <button onClick={handleGenerateWeather} className="bg-amber-500 text-slate-900 font-bold py-2 px-6 rounded-md shadow-md hover:bg-amber-400">Tirar 2D6</button>
                    </div>
                </div>
            )}
            {isChangingWeatherModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={() => setIsChangingWeatherModalOpen(false)}>
                    <div className="bg-slate-800 p-6 rounded-lg shadow-xl border border-slate-700 max-w-sm w-full text-center" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-bold text-amber-400 mb-4">¡Clima Cambiante!</h3>
                        <p className="text-slate-300 mb-4">Haz una nueva tirada en la Tabla de Clima (2D6) e introduce el resultado.</p>
                        <input type="text" pattern="([2-9]|1[0-2])" value={weatherRerollInput} onChange={e => setWeatherRerollInput(e.target.value.replace(/[^0-9]/g, '').slice(0, 2))} className="w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white mb-4 text-center" placeholder="Resultado 2D6 (2-12)" autoFocus />
                        <button onClick={handleConfirmWeatherReroll} className="bg-amber-500 text-slate-900 font-bold py-2 px-6 rounded-md shadow-md hover:bg-amber-400">Confirmar Clima</button>
                    </div>
                </div>
            )}
            {isTdModalOpen && liveHomeTeam && liveOpponentTeam && (
                <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={() => setIsTdModalOpen(false)}>
                    <div className="bg-slate-800 rounded-lg shadow-xl border border-slate-700 max-w-md w-full" onClick={e => e.stopPropagation()}>
                        <h2 className="text-xl font-bold text-amber-400 p-4 border-b border-slate-700">Anotar Touchdown</h2>
                        {!tdModalTeam ? (
                            <div className="p-5 space-y-3">
                                <h3 className="text-lg text-slate-300 mb-4">¿Qué equipo ha anotado?</h3>
                            ...
                            <div className="p-5 space-y-2 max-h-[60vh] overflow-y-auto">
                                <h3 className="text-lg text-slate-300 mb-4">¿Qué jugador ha anotado?</h3>
                                {(tdModalTeam === 'home' ? liveHomeTeam : liveOpponentTeam).players.filter(p => p.status === 'Activo').map(p => <PlayerButton key={p.id} player={p} onSelect={handleSelectTdScorer} />)}
                            </div>
                        )}
                    </div>
                </div>
            ...
            {sppModalState.isOpen && liveHomeTeam && liveOpponentTeam && (
                <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={() => setSppModalState({ isOpen: false, type: null, step: 'select_team', teamId: null, selectedPlayer: null })}>
                    <div className="bg-slate-800 rounded-lg shadow-xl border border-slate-700 max-w-md w-full" onClick={e => e.stopPropagation()}>
                        <h2 className="text-xl font-bold text-amber-400 p-4 border-b border-slate-700">Añadir PE por {sppModalState.type === 'pass' ? 'Pase' : sppModalState.type === 'interference' ? 'Interferencia' : 'Lesión'}</h2>
                        <div className="p-5">
            ...
            {isCustomEventModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={() => setIsCustomEventModalOpen(false)}>
                    <div className="bg-slate-800 rounded-lg shadow-xl border border-slate-700 max-w-md w-full" onClick={e => e.stopPropagation()}>
                        <h2 className="text-xl font-bold text-amber-400 p-4 border-b border-slate-700">Registrar Evento Personalizado</h2>
                        <div className="p-5">
                            <textarea value={customEventDescription} onChange={e => setCustomEventDescription(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white" rows={3} placeholder="Describe el evento..."></textarea>
                        </div>
                        <div className="p-4 bg-slate-900/50 border-t border-slate-700 flex justify-end">
                            <button onClick={() => { logEvent('OTHER', customEventDescription); setIsCustomEventModalOpen(false); setCustomEventDescription(''); }} className="bg-amber-500 text-slate-900 font-bold py-2 px-6 rounded-md">Guardar</button>
                        </div>
                    </div>
                </div>
            )}
