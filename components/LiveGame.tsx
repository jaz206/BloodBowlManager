...
const PlayerButton: React.FC<{ player: ManagedPlayer, onSelect: (p: ManagedPlayer) => void, disabled?: boolean }> = ({ player, onSelect, disabled }) => (
    <button onClick={() => onSelect(player)} className="w-full text-left bg-slate-700/50 p-3 rounded-md hover:bg-slate-700 transition-colors disabled:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50" disabled={disabled}>
...
export const LiveGame = ({ managedTeams, onTeamsUpdate }: LiveGameProps): React.ReactElement => {
    ...
    useEffect(() => { if (homeTeam) { const liveTeam = JSON.parse(JSON.stringify(homeTeam)); liveTeam.players.forEach((p: ManagedPlayer) => { if (!p.status) p.status = 'Reserva'; if (!p.sppActions) p.sppActions = {}; }); liveTeam.liveRerolls = liveTeam.rerolls; setLiveHomeTeam(liveTeam); } }, [homeTeam]);
    useEffect(() => { if (opponentTeam) { const liveTeam = JSON.parse(JSON.stringify(opponentTeam)); liveTeam.players.forEach((p: ManagedPlayer) => { if (!p.status) p.status = 'Reserva'; if (!p.sppActions) p.sppActions = {}; }); liveTeam.liveRerolls = liveTeam.rerolls; setLiveOpponentTeam(liveTeam); } }, [opponentTeam]);
    ...
    const handleConfirmPostGame = (finalTeamState: ManagedTeam) => { if (!homeTeam) return; const allTeams = managedTeams.map(t => t.name === finalTeamState.name ? finalTeamState : t); onTeamsUpdate(allTeams); setGameState('setup'); setHomeTeam(null); setOpponentTeam(null); setLiveHomeTeam(null); setLiveOpponentTeam(null); setGameLog([]); setScore({home: 0, opponent: 0}); setTurn(0); setHalf(1); setFame({ home: 0, opponent: 0 }); setFansRoll({ home: '', opponent: ''}); };
    ...
    const handleHireStar = (star: StarPlayer) => { if (!inducementState.underdog) return; const newPlayer: ManagedPlayer = { id: Date.now() + Math.random(), customName: star.name, position: "Jugador Estrella", cost: star.cost, stats: star.stats, skills: star.skills, primary: 'G', secondary: 'A,F,P', spp: 0, gainedSkills: [], lastingInjuries: [], isStarPlayer: true, qty: "0-1", status: 'Reserva' }; const setUnderdogTeam = inducementState.underdog === 'home' ? setLiveHomeTeam : setLiveOpponentTeam; const underdogTeam = inducementState.underdog === 'home' ? liveHomeTeam : liveOpponentTeam; setUnderdogTeam(prev => prev ? ({ ...prev, players: [...prev.players, newPlayer] }) : null); setInducementState(prev => ({ ...prev, money: prev.money - star.cost, hiredStars: [...prev.hiredStars, newPlayer] })); if (underdogTeam) logEvent('INFO', `${underdogTeam.name} contrata a ${star.name} por ${star.cost.toLocaleString()} M.O.`); };
    const handleFireStar = (star: ManagedPlayer) => { if (!inducementState.underdog) return; const setUnderdogTeam = inducementState.underdog === 'home' ? setLiveHomeTeam : setLiveOpponentTeam; const underdogTeam = inducementState.underdog === 'home' ? liveHomeTeam : liveOpponentTeam; setUnderdogTeam(prev => prev ? ({ ...prev, players: prev.players.filter(p => p.id !== star.id) }) : null); setInducementState(prev => ({ ...prev, money: prev.money + star.cost, hiredStars: prev.hiredStars.filter(s => s.id !== star.id) })); if (underdogTeam) logEvent('INFO', `${underdogTeam.name} despide a ${star.customName} y recupera ${star.cost.toLocaleString()} M.O.`); };
    const handleBuyInducement = (item: 'reroll' | 'bribe' | 'cheerleader' | 'coach', cost: number) => { if (inducementState.money < cost) { alert("Fondos insuficientes."); return; } const setUnderdogTeam = inducementState.underdog === 'home' ? setLiveHomeTeam : setLiveOpponentTeam; setInducementState(prev => ({ ...prev, money: prev.money - cost })); setUnderdogTeam(prev => { if (!prev) return null; const newTeam = { ...prev }; if (item === 'reroll') newTeam.liveRerolls = (newTeam.liveRerolls || 0) + 1; if (item === 'bribe') newTeam.tempBribes = (newTeam.tempBribes || 0) + 1; if (item === 'cheerleader') newTeam.tempCheerleaders = (newTeam.tempCheerleaders || 0) + 1; if (item === 'coach') newTeam.tempAssistantCoaches = (newTeam.tempAssistantCoaches || 0) + 1; return newTeam; }); };
    const handleSellInducement = (item: 'reroll' | 'bribe' | 'cheerleader' | 'coach', cost: number) => { const underdogTeam = inducementState.underdog === 'home' ? liveHomeTeam : liveOpponentTeam; if (!underdogTeam) return; const canSell = (item === 'reroll' && (underdogTeam.liveRerolls || 0) > underdogTeam.rerolls) || (item === 'bribe' && (underdogTeam.tempBribes || 0) > 0) || (item === 'cheerleader' && (underdogTeam.tempCheerleaders || 0) > 0) || (item === 'coach' && (underdogTeam.tempAssistantCoaches || 0) > 0); if (!canSell) { alert("No tienes de este incentivo para vender."); return; } const setUnderdogTeam = inducementState.underdog === 'home' ? setLiveHomeTeam : setLiveOpponentTeam; setInducementState(prev => ({ ...prev, money: prev.money + cost })); setUnderdogTeam(prev => { if (!prev) return null; const newTeam = { ...prev }; if (item === 'reroll') newTeam.liveRerolls = (newTeam.liveRerolls || 1) - 1; if (item === 'bribe') newTeam.tempBribes = (newTeam.tempBribes || 1) - 1; if (item === 'cheerleader') newTeam.tempCheerleaders = (newTeam.tempCheerleaders || 1) - 1; if (item === 'coach') newTeam.tempAssistantCoaches = (newTeam.tempAssistantCoaches || 1) - 1; return newTeam; }); };
                ...
                <div className="text-center p-4 sm:p-8 max-w-md mx-auto">
                    ...
                    <div className="space-y-3">
                        {managedTeams.map(team => (
                            <button key={team.name} onClick={() => { setHomeTeam(team); setGameState(hasCamera ? 'scanning' : 'manual_select'); }} className="w-full bg-slate-700/50 text-slate-200 font-semibold p-4 rounded-lg shadow-md hover:bg-slate-700 hover:text-white">
            ...
            case 'manual_select': return (
                 <div className="text-center p-4 sm:p-8 max-w-md mx-auto">
                    <h2 className="text-3xl font-bold text-amber-400 mb-4">Elige al Oponente</h2>
                     <div className="space-y-3">
                         {managedTeams.filter(t => t.name !== homeTeam?.name).map(team => ( <button key={team.name} onClick={() => handleManualOpponentSelect(team.name)} className="w-full bg-slate-700/50 text-slate-200 font-semibold p-4 rounded-lg shadow-md hover:bg-slate-700 hover:text-white">{team.name}</button> ))}
                    </div>
                </div>
            );
            case 'pre_game': {
                                    ...
                                    {[liveHomeTeam, liveOpponentTeam].map((team, index) => {
                                        ...
                                        return (
                                            <div key={teamId} className="bg-slate-800 p-3 rounded-lg flex flex-col">
                                                <h3 className={`font-bold text-lg mb-2 ${teamId === 'home' ? 'text-sky-400' : 'text-red-400'}`}>{team.name}</h3>
                ...
                <div className="space-y-4">
                    <div className="bg-slate-900/70 p-4 rounded-lg border border-slate-700">
                         <div className="grid grid-cols-3 items-center text-center">
                              <h3 className="text-lg sm:text-xl font-bold text-sky-400 truncate">{liveHomeTeam.name}</h3>
                              <div className="font-bold text-lg text-amber-400">Parte {half} - Turno {turn}</div>
                              <h3 className="text-lg sm:text-xl font-bold text-red-400 truncate">{liveOpponentTeam.name}</h3>
                            </div>
                            <div className="text-5xl sm:text-7xl font-black text-white text-center my-2 tracking-wider" style={{ textShadow: '2px 2px 8px rgba(0,0,0,0.7)' }}>{score.home} - {score.opponent}</div>
                                    ...
                                    <div key={event.id} className="border-b border-slate-800 pb-1 last:border-b-0">
                                        <p>
                                            ...
                                            <span className={`font-semibold ${
                                                event.type === 'TURNOVER' ? 'text-red-400' : 
