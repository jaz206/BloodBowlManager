...
const UserProfile: React.FC = () => {
  const { user, logout, isLoading, isGsiInitialized } = useAuth();
        ...
        <div className="flex items-center gap-2">
            ...
            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-amber-400 rounded-full">
                <img
                    src={user.picture}
                    alt="User avatar"
                    className="w-10 h-10 rounded-full border-2 border-slate-600 hover:border-amber-400 transition-colors"
                />
            </button>
            {isDropdownOpen && (
         ...
         isGsiInitialized ? (
            ...
            <div className="text-xs text-slate-400 p-2 text-center bg-slate-800 rounded-md border border-slate-700">
                <p className="font-semibold text-amber-400">Google Sign-In no configurado.</p>
                <p className="mt-1">Añada su ID de cliente en <code>index.html</code>.</p>
            </div>
         )
      )}
      <style>{`
