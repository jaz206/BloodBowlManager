...
const StarPlayerCard: React.FC<{ player: StarPlayer }> = ({ player }) => {
    return (
        <div className="bg-slate-800/80 backdrop-blur-sm p-4 rounded-xl border border-white/5 hover:border-premium-gold/30 transition-all group overflow-hidden">
            <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-2 mt-2 text-[10px] items-center">
                        <div className="flex flex-col items-center bg-black/40 px-2 py-0.5 rounded border border-white/5">
                            <span className="text-slate-500 font-bold">MV</span>
                            <span className="text-white font-mono">{player.stats.MV}</span>
                        </div>
                        <div className="flex flex-col items-center bg-black/40 px-2 py-0.5 rounded border border-white/5">
                            <span className="text-slate-500 font-bold">FU</span>
                            <span className="text-white font-mono">{player.stats.FU}</span>
                        </div>
                        <div className="flex flex-col items-center bg-black/40 px-2 py-0.5 rounded border border-white/5">
                            <span className="text-slate-500 font-bold">AG</span>
                            <span className="text-white font-mono">{player.stats.AG}</span>
                        </div>
                        <div className="flex flex-col items-center bg-black/40 px-2 py-0.5 rounded border border-white/5">
                            <span className="text-slate-500 font-bold">PA</span>
                            <span className="text-white font-mono">{player.stats.PS}</span>
                        </div>
                        <div className="flex flex-col items-center bg-black/40 px-2 py-0.5 rounded border border-white/5">
                            <span className="text-slate-500 font-bold">AR</span>
                            <span className="text-white font-mono">{player.stats.AR}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-4">
                ...
                {player.specialRules && (
                    <p className="text-[10px] text-slate-400 mt-2 italic bg-black/20 p-2 rounded">
...
const StarPlayers: React.FC = () => {
    ...
    const filteredPlayers = useMemo(() => {
        return starPlayersData.filter(p => {
            ...
            const matchesFaction = selectedFaction === 'Todas' || p.playsFor.includes(selectedFaction) || p.playsFor.includes('Any Team') || p.playsFor.includes('Any team except Sylvanian Spotlight');
